...
const sectionGroups: Record<string, (typeof sectionLinks)[number]["id"]> = {
  hero: "hero",
  about: "hero",
  work: "work",
...
export default function Navbar() {
  ...
  useEffect(() => {
    const sections = Object.keys(sectionGroups).map((id) => document.getElementById(id)).filter(Boolean) as HTMLElement[];
    const observer = new IntersectionObserver((entries) => {
      const visible = entries.filter((entry) => entry.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0];
      if (visible?.target.id) setActiveSection(sectionGroups[visible.target.id] ?? "hero");
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-3 z-30 sm:bottom-4">
      <Dock magnification={58} className="liquid-dock pointer-events-auto relative z-50 mx-auto flex h-14 w-fit gap-1 rounded-full border bg-card/90 p-1.5 shadow-[0_10px_30px_-15px_rgba(0,0,0,.28)] backdrop-blur-3xl sm:h-[60px] sm:gap-2 sm:p-2">
        ...
        {Object.entries(DATA.contact.social).filter(([_, social]) => social.navbar).map(([name, social]) => {
          ...
          return <DockTip key={name} label={name}><a href={social.url} target={social.url.startsWith("http") ? "_blank" : undefined} rel={social.url.startsWith("http") ? "noopener noreferrer" : undefined} aria-label={name} className="hidden sm:block"><DockIcon className={dockIconClass}><Icon className="size-full" /></DockIcon></a></DockTip>;
        })}
