...
export default function Page() {
  return (
    <main className="min-h-dvh flex flex-col gap-14 sm:gap-16 relative">
      ...
      <section id="skills">
        <div className="flex min-h-0 flex-col gap-y-5">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {DATA.skills.map((skill, id) => (<BlurFade key={skill.name} delay={BLUR_FADE_DELAY * 14 + id * 0.025}><div className="group flex min-h-14 items-center gap-3 rounded-xl border bg-card px-3.5 py-3 transition-all duration-300 hover:-translate-y-0.5 hover:border-foreground/15 hover:shadow-sm"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-muted/60 transition-colors duration-300 group-hover:bg-foreground group-hover:text-background"><skill.icon className="size-4" /></span><span className="text-sm font-medium leading-snug text-foreground/85">{skill.name}</span></div></BlurFade>))}
          </div>
        </div>
      </section>
      ...
      <section id="brands">
        <div className="flex min-h-0 flex-col gap-y-5">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {DATA.brands.map((brand, index) => (<BlurFade key={brand.name} delay={BLUR_FADE_DELAY * 16 + index * 0.025}><div className="group flex h-24 flex-col items-center justify-center gap-2 rounded-2xl border bg-white p-3 shadow-sm ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-1 hover:border-foreground/15 hover:shadow-md dark:bg-white/95"><img src={brand.logo} alt={`${brand.name} logo`} loading="lazy" className="max-h-10 max-w-[104px] object-contain contrast-125 saturate-110 transition-transform duration-300 group-hover:scale-105" /><span className="text-[10px] font-medium text-neutral-600">{brand.name}</span></div></BlurFade>))}
          </div>
        </div>
      </section>
        ...
        <div className="flex min-h-0 flex-col gap-y-5">
          ...
          <BlurFade delay={BLUR_FADE_DELAY * 20}>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <div className="rounded-2xl border bg-card p-4"><p className="text-sm font-semibold">Markets</p><p className="mt-2 text-xs leading-relaxed text-foreground/65">{DATA.markets.join(" · ")}</p></div>
              </div>
              <div>
                <div className="rounded-2xl border bg-card p-4"><p className="text-sm font-semibold">Industries</p><p className="mt-2 text-xs leading-relaxed text-foreground/65">{DATA.industries.join(" · ")}</p></div>
              </div>
            </div>
          </BlurFade>
        </div>
      ...
      <section id="education"><div className="flex min-h-0 flex-col gap-y-6"><BlurFade delay={BLUR_FADE_DELAY * 25}><h2 className="text-xl font-bold">Education & Additional Information</h2></BlurFade>{DATA.education.map((education) => (<BlurFade key={education.school} delay={BLUR_FADE_DELAY * 26}><Link href={education.href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-x-3 justify-between rounded-xl border p-3 group transition-colors hover:bg-muted/30"><div className="flex items-center gap-x-3 flex-1 min-w-0"><div className="size-9 border rounded-full shadow ring-2 ring-border bg-muted flex-none flex items-center justify-center text-xs font-semibold">BS</div><div className="flex-1 min-w-0 flex flex-col gap-0.5"><div className="font-semibold leading-none flex items-center gap-2">{education.school}<ArrowUpRight className="h-3.5 w-3.5 text-muted-foreground opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200" /></div><div className="font-sans text-sm text-muted-foreground">{education.degree}</div></div></div><div className="text-xs tabular-nums text-muted-foreground text-right flex-none">{education.end}</div></Link></BlurFade>))}<div className="grid gap-2">{DATA.additional.map((item, index) => (<BlurFade key={item.label} delay={BLUR_FADE_DELAY * 27 + index * 0.04}><div className="rounded-xl border p-3 transition-colors duration-200 hover:bg-muted/30"><p className="text-xs font-medium text-muted-foreground">{item.label}</p><p className="mt-1 text-sm font-medium">{item.value}</p></div></BlurFade>))}</div></div></section>
