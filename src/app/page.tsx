          ...
          <div className="gap-5 gap-y-6 flex flex-col md:flex-row justify-between">
            <div className="gap-3 flex flex-col order-2 md:order-1">
              ...
              <BlurFadeText
                ...
                text="I build paid media systems that turn ad spend into measurable, scalable growth."
              />
              ...
              <BlurFade delay={BLUR_FADE_DELAY * 6}>
                <div className="mt-4 flex flex-wrap gap-2.5">
                  ...
                  <a href="https://sherifseniormediabuyer-shiffo900s-projects.vercel.app/" target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-2 rounded-xl border border-sky-400/25 bg-sky-500/10 px-4 py-2.5 text-sm font-semibold text-sky-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-sky-500/15 dark:text-sky-300">
                    <ExternalLink className="size-4" /> View Full Portfolio
      ...
      <section id="brands" className="overflow-hidden">
        <div className="flex min-h-0 flex-col gap-y-5">
          ...
          <BlurFade delay={BLUR_FADE_DELAY * 16}>
            <div className="brand-marquee relative overflow-hidden -mx-2 px-2"><div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-10 bg-gradient-to-r from-background to-transparent" /><div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-10 bg-gradient-to-l from-background to-transparent" /><div className="brand-marquee-track flex w-max gap-3 py-2">{[...DATA.brands, ...DATA.brands].map((brand, index) => (<div key={`${brand.name}-${index}`} aria-hidden={index >= DATA.brands.length} className="group flex h-20 w-32 shrink-0 flex-col items-center justify-center gap-1.5 rounded-2xl border bg-white p-3 shadow-sm ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:bg-white/95"><img src={brand.logo} alt={index < DATA.brands.length ? `${brand.name} logo` : ""} loading="lazy" className="max-h-9 max-w-[96px] object-contain contrast-125 saturate-110 transition-transform duration-300 group-hover:scale-[1.04]" /><span className="text-[10px] font-medium text-neutral-600">{brand.name}</span></div>))}</div></div>
          </BlurFade>
        </div>
      </section>
      ...
      <section id="systems">
        <div className="flex min-h-0 flex-col gap-y-6">
          <BlurFade delay={BLUR_FADE_DELAY * 19}>
            <div>
              <p className="text-xs uppercase tracking-[0.26em] text-muted-foreground font-medium">Operating Stack</p>
              <h2 className="mt-2 text-xl font-bold">Capabilities &amp; Tools</h2>
              <p className="mt-1 text-sm text-foreground/65">The systems, platforms, markets, and workflows behind the performance work.</p>
            </div>
          </BlurFade>
          ...
          <div className="flex flex-col gap-4 border-t pt-5">
            {DATA.toolGroups.map((group, index) => (
              <BlurFade key={group.label} delay={BLUR_FADE_DELAY * 21 + index * 0.04}>
                <div className="grid gap-2 sm:grid-cols-[138px_1fr]">
          ...
          <BlurFade delay={BLUR_FADE_DELAY * 25}>
            <div className="grid gap-5 border-t pt-5 sm:grid-cols-2">
              <div>
                ...
                <p className="mt-2 text-xs leading-relaxed text-foreground/65">{DATA.markets.join(" · ")}</p>
              </div>
              <div>
                <p className="text-sm font-semibold">Industries</p>
                <p className="mt-2 text-xs leading-relaxed text-foreground/65">{DATA.industries.join(" · ")}</p>
              </div>
            </div>
          </BlurFade>
        </div>
